...
var YouTube = function() {
	...
	self.getCommentThreadsByVideoId = function(videoId, callback, pageToken) {
		...
		if (validate !== null) {
			...
			self.request(self.getUrl('commentThreads'), function(err, res){
...
/**
 * Get data for video category by id
 * @param {string} id
 * @param {function} callback
 * https://developers.google.com/youtube/v3/docs/playlists/list
 */
self.getVideoCategoryById = function(id, callback) {
	var validate = self.validate();
	...
	if (validate !== null) {
		...
		self.request(self.getUrl('videoCategories'), callback);
	}
};
